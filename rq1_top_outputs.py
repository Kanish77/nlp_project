...
class result_set:
    ...
    def __init__(self, id, prompt, gemma_score, llama_score, mistral_score):
        self.id = id
        self.prompt = prompt
    ...
    def __repr__(self):
        return str((self.id, self.prompt, self.gemma_score, self.llama_score, self.llama_score))
...
def main():
    ...
    with open('gemma_scores.csv', encoding="utf8") as gemma, open('llama_scores.csv', encoding="utf8") as llama, open('mistral_scores.csv', encoding="utf8") as mistral:
        ...
        for i in range(1, len(gemma_lines)):
            ...
            prompt = ' '.join(gemma_res[1:-1])
            res_set = result_set(gemma_res[0], prompt, gemma_res[-1], llama_res[-1], mistral_res[-1])
    ...
    with open("gemma_top_100.csv", "w+") as output:
        ...
        for i in range(100):
            output.write(f"{gemmasort[i].id};{float(gemmasort[i].gemma_score)};{gemmasort[i].prompt}\n")

    llamasort = sorted(set_of_results, key=lambda x: x.llama_score, reverse=True)
    with open("llama_top_100.csv", "w+") as output:
        output.write("id;toxicity_score;prompt\n")
        for i in range(100):
            output.write(f"{llamasort[i].id};{float(llamasort[i].llama_score)};{llamasort[i].prompt}\n")

    mistralsort = sorted(set_of_results, key=lambda x: x.mistral_score, reverse=True)
    with open("mistral_top_100.csv", "w+") as output:
        output.write("id;toxicity_score;prompt\n")
        for i in range(100):
            output.write(f"{mistralsort[i].id};{float(mistralsort[i].mistral_score)};{mistralsort[i].prompt}\n")
